...
export default function ReportedMessages() {
  ...
  const load = useCallback(async (off = offset) => {
    setLoading(true);
    try {
      const { items, total } = await api.reportedMessages.list({ limit: LIMIT, offset: off });
      setItems(items);
      setTotal(total);
    } catch (e) {
  ...
  async function handleDelete(item) {
    try {
      await api.reportedMessages.delete(item.id);
              ...
              <tbody>
                {items.map(it => (
                  <tr key={it.id} style={{ cursor: 'pointer' }} onClick={() => setViewing(it)}>
                    <td>{it.reporter_name || <span style={{ color: 'var(--text-muted)' }}>—</span>}</td>
                    <td>{it.reported_player || <span style={{ color: 'var(--text-muted)' }}>—</span>}</td>
        ...
        <div className="modal-overlay" onClick={() => setViewing(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{viewing.reported_player || 'Mensaje reportado'}</h2>
              <button className="modal-close" onClick={() => setViewing(null)}>✕</button>
            </div>
            <div className="modal-body">
              <div className="form-group">
                <label>Reportado por</label>
                <p>{viewing.reporter_name || '—'}</p>
              </div>
              <div className="form-group">
                <label>Fecha</label>
              ...
              <div className="form-group">
                <label>Mensaje</label>
                <p style={{ whiteSpace: 'pre-wrap' }}>{viewing.message_text}</p>
              </div>
            </div>
